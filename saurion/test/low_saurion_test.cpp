  ...
  void SetUp() override {
    ...
    saurion
        ...
        ->on_closed(
            ...
            nullptr)
        ->on_error([](int, const char *const, const ssize_t, void *) -> void {}, nullptr);
    saurion->init();
    LOG_END("");
  }
...
char *SaurionTest::fifo_name = nullptr;
std::thread *SaurionTest::sender = nullptr;
FILE *SaurionTest::fifo_write = nullptr;

TEST_F(SaurionTest, connectMultipleClients) {
  uint32_t clients = 10;
  connect_clients(clients);
  wait_connected(clients);
  EXPECT_EQ(summary.connected, clients);
...
TEST_F(SaurionTest, readMultipleMsgsFromClients) {
  LOG_INIT("");
  uint32_t clients = 20;
  uint32_t msgs = 100;
  connect_clients(clients);
...
TEST_F(SaurionTest, writeMsgsToClients) {
  uint32_t clients = 20;
  uint32_t msgs = 100;
  connect_clients(clients);
  wait_connected(clients);
...
TEST_F(SaurionTest, reconnectClients) {
  uint32_t clients = 5;
  connect_clients(clients);
  wait_connected(clients);
  EXPECT_EQ(summary.connected, clients);
...
TEST_F(SaurionTest, readWriteWithLargeMessage) {
  uint32_t clients = 1;
  size_t size = CHUNK_SZ * 10;
  char *str = new char[size + 1];
  memset(str, 'A', size);
...
TEST_F(SaurionTest, handleConcurrentReadsAndWrites) {
  uint32_t clients = 20;
  uint32_t msgs = 100;
  connect_clients(clients);
  wait_connected(clients);
...
TEST_F(SaurionTest, largeNumberOfMessages) {
  uint32_t clients = 10;
  uint32_t msgs = 1000;
  connect_clients(clients);
  wait_connected(clients);
