 ...
 *
 * Esta estructura contiene todos los datos necesarios para manejar la cola de
 * eventos de io_uring, así como los callbacks para los eventos de socket.
 */
struct saurion {
  struct io_uring *rings;   /**< Array de estructuras io_uring para manejar la
                               cola de eventos. */
  ...
  struct Node
      *list; /**< Lista enlazada para almacenar las solicitudes en curso. */
  pthread_mutex_t
      status_m; /**< Mutex para proteger el estado de la estructura. */
  pthread_cond_t status_c; /**< Condición para señalar cambios en el estado. */
  int status;              /**< Estado actual de la estructura. */
  ThreadPool
      *pool; /**< Pool de Threads para ejecutar en paralelo las operaciones */
  ...
  struct saurion_callbacks {
    ...
    void *on_connected_arg; /**< Argumento adicional para el callback de
                               conexión. */
    ...
    void
        *on_readed_arg; /**< Argumento adicional para el callback de lectura. */
    ...
    void *
        on_wrote_arg; /**< Argumento adicional para el callback de escritura. */
     ...
     * @param arg Argumento adicional proporcionado por el usuario.
     */
    void (*on_error)(const int fd, const char *const content, const ssize_t len,
                     void *arg);
 ...
 *
 * Esta función inicializa la estructura saurion, configura el eventfd y la cola
 * de io_uring, y prepara la estructura para su uso.
 *
 * @return struct saurion* Puntero a la estructura saurion creada, o NULL en
 * caso de error.
 */
[[nodiscard]]
 ...
 *
 * Esta función inicia la aceptación de conexiones y el procesamiento de eventos
 * de io_uring. Se ejecuta en un bucle hasta que se recibe una señal de parada.
 *
 ...
 *
 * Esta función envía una señal al eventfd para indicar que se debe detener el
 * bucle de eventos.
 *
 * @param s Puntero a la estructura saurion.
 */
 ...
 *
 * Esta función espera a que se detenga el procesamiento de eventos, libera la
 * memoria de la estructura saurion y cierra los descriptores de archivo.
 *
 * @param s Puntero a la estructura saurion.
 */
 ...
 *
 * Esta función prepara y envía un mensaje a través del socket especificado
 * utilizando la cola de eventos de io_uring.
 *
 * @param s Puntero a la estructura saurion.
